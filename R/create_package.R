...
create_package <- function(package, path = ".") {
  ...
  c("# Generated by roxygen2: do not edit by hand", "") |>
    writeLines(file.path(path, "NAMESPACE"))
  ...
  dir.create(file.path(path, target), recursive = TRUE, showWarnings = FALSE)
  insert_file(
    repo = repo,
...
insert_file <- function(repo, filename, template, target = ".", new_name) {
  ...
  if (target != ".") {
    new_name <- file.path(target, new_name)
  }
  file.path(template, filename) |>
    system.file(package = "checklist") |>
    file.copy(file.path(repo, new_name), overwrite = TRUE)
  if (is.null(repo)) {
    return(invisible(NULL))
  }
