...
create_package <- function(package, path = ".") {
  ...
  assert_that(
    file_test("-d", path),
    msg = sprintf("`%s` is not a directory", path)
  )
  assert_that(is.string(package))
  assert_that(valid_package_name(package))
  path <- path_(path, package)
  assert_that(
    !file_test("-d", path) || length(list.files(path, recursive = TRUE)) == 0,
  ...
  dir.create(path_(path, "R"), recursive = TRUE, showWarnings = FALSE)
  ...
  # Add code of conduct
  dir.create(path_(path, ".github"), recursive = TRUE, showWarnings = FALSE)
  insert_file(
    repo = repo,
  ...
  dir.create(path_(path, target), recursive = TRUE, showWarnings = FALSE)
  insert_file(
    repo = repo,
...
insert_file <- function(repo, filename, template, target = ".", new_name) {
  ...
  if (target != ".") {
    new_name <- path_(target, new_name)
  }
  path_(template, filename) |>
    system.file(package = "checklist") |>
    file.copy(path_(repo, new_name), overwrite = TRUE)
  if (is.null(repo)) {
    return(invisible(NULL))
  }
