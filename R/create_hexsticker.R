...
create_hexsticker <-
  ...
  if (requireNamespace("hexSticker") & requireNamespace("extrafont") &
      requireNamespace("sysfonts")) {
    background <- system.file("inbo-empty.png", package = "checklist")
    if ("FlandersArtSans-Medium" %in% extrafont::fonts()) {
      sysfonts::font_add(family = "Flanders Art Sans",
                         regular = "FlandersArtSans-Medium.otf")
    }
    hexSticker::sticker(
