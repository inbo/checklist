spelling_parse_r <- function(r_file, wordlist) {
  ...
  # remove empty roxygen
  text <- gsub("^#'\\s*$", "", text)
  # remove @param
  text <- gsub("^#'\\s*@param\\s+[\\w|\\.]+", "", text, perl = TRUE)
  # remove @export
  text <- gsub("^#'\\s*@export", "", text)
  # remove @inheritParams
  text <- gsub("^#'\\s*@inheritParams .*", "", text)
  # remove @importFrom
  text <- gsub("^#'\\s*@importFrom .*", "", text)
  # remove @author
  text <- gsub("^#'\\s*@author .*", "", text)
  # remove @keywords
  text <- gsub("^#'\\s*@keywords .*", "", text)
  # remove @concept
  text <- gsub("^#'\\s*@concept", "", text)
  # remove @return
  text <- gsub("^#'\\s*@return", "", text)
  # remove @family
  text <- gsub("^#'\\s*@family .*", "", text)
  # remove backticks
  text <- gsub("`.*?`", "", text)
