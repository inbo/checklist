...
1. I want to use or update `checklist` in an R project containing [only R scripts or markdown file](#setup-project).
...
1. Run `rorcid::orcid_auth()` in the R console and sign in with your [ORCID iD](https://orcid.org) if requested.
...
## Adding or updating checklist support to a project with R scripts {#setup-project}

Below describes the legacy version using `setup_source()`.
This functionality will gradually migrate into `setup_project()`.
Use `setup_project()` when you want granular control on which checks to run.
...
1. Install the checklist package with `remotes::install_github("inbo/checklist")`.
1. Run `checklist::setup_source(language = "en-GB")`.
Change the language to the one you want to use as default.
...
Repeat this step until you have only issues that are not a problem.
Commit any changes.
1. Update the `checklist.yml` with `checklist::write_checklist(x, package = FALSE)`.
This function will ask you which (if any) of the current allowed warnings and notes you want to keep.
Then it asks which warnings and notes you want to allow.
You'll need to motivate each of them.
...
Sometimes the check package GitHub Action (GHA) may fail for other reasons than failure of one or more of the checks executed by `checklist::check_package()`. This is likely due to a missing dependency when the action tries to install R packages.
...
- Search the log (CTRL+F) using the name of the package
  - Scroll down to the last found instance of the package name and make your way up
