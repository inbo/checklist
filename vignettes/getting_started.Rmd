...
If your code depends on an R packages that is not on CRAN, then you need to let `checklist` know where to find it.
...
To diagnose if the failed action is due to failing to install dependencies, you can follow these steps:
...
Should you need to run Linux commands to install the missing system dependencies, you can add these commands to the `checklist.yml` file.
You can run any Linux command.
Be carefull when doing this, because you can break the `GHA` if you add wrong commands.
...
Add, commit and push changes in `checklist.yml` so they take effect in the `GHAs`.
