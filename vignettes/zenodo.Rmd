...
## What is [Zenodo](https://www.zenodo.org)?
...
- The code gets a DOI, making it easier to cite the code and track the citations.
  Every release gets is own release and there is an dedicated DOI which always points to the latest release.
  This gives the user a choice between citing the code in general or a specific version.

## What is [ORCID](https://orcid.org)?

ORCID, which stands for **O**pen **R**esearcher and **C**ontributor **ID**, is a global, not-for-profit organization sustained by fees from their member organizations.
...
The researcher can add manulay add publications to this profile.
...
Setting up the integration has the benefit that a new package version automatically get a unique DOI and is added to your ORCID profile.
...
1. Active [DataCite](https://support.datacite.org/docs/how-to-activate-orcid-auto-update) automatic updates.
...
1. Link your GitHub to your Zenodo profile.

#### Once per repository

1. Log-in to Zendo.
1. Go the to dropdown box associated to your account (top right) and choose GitHub.
...
1. Make sure that you've set-up your code an package with `checklist` support.
...
1. Merging a pull request to the main branch will trigger a GitHub Action workflow that add a new tag with the version number.
...
1. Zenodo passes the citation metadata to ORCID for all contributors with an ORCID listed in `DESCRIPTION`.
  ORCID adds the publication information to the users which have added the DateCite integration.
