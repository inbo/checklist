...
When `check_spelling()` finds a [Quarto book or website](https://quarto.org), it will check if `lang` is set in `_quarto.yml`.
Then `check_spelling()` uses that `lang` for all files listed under the `book:` or `website:` section.
