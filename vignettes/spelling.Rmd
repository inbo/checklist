...
Or run `check_spelling(path)` were `path` points to a directory with the files you want to spell check.
...
You have a few options to handles those words in `.Rmd` files.
...
End the block with `<!-- spell-check: ignore:end-->` on a new line at the end of the block.
...
1. Add the words to a project specific custom dictionary.

## Custom dictionary

First of all make sure that `check_spelling()` only return words you want to add to the custom dictionary.
...
When this file is missing, `check_spelling()` assumes your spelling is an R package.
It will use the language set in the `DESCRIPTION` as default language.
In case the `Language:` field is not set or when there is no `DESCRIPTION`, `en-GB` is used.

Note that a language is defined with the `xx-YY` syntax.
The first two letters must be lower case (`xx`) and define the language.
While the last two letters must be upper case (`YY`) and define the language variant.
...
Before you can change the setting, you must read the current settings with `read_checklist(path)`.
This object has several methods to change the spell checking settings.
`$set_default(language)` sets the default language.
`$set_exceptions()` allows you to review the current settings on the relevant folders and files within the project.
...
Therefore the function gives an extra option in case of a folder: change the setting for some of the files.
After updating the settings, you need to store them with `write_checklist()`.
...
If you run `read_checklist()` on a folder without a `checklist.yml`, it will return a default `checklist.yml`.
