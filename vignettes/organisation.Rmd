...
Such a rule limits to use of `checklist` outside of INBO.
...
The store this object in your project with `write_organisation()`.
...
When `read_organisation()` find neither a project `organisation.yml` nor a machines default `organisation.yml`, it reverts to the INBO `organisation`.
