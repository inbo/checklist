...
Everybody develops their own coding habits and style.
...
The `checklist` package defines a set of standards and provides tools to valid whether your project or R package adheres to these standards.
You can run these tools interactive on your machine.
You can also add this checks as [GitHub actions](https://github.com/features/actions), which runs then checks automatically after every [push](https://github.com/git-guides/git-push) to the repository on [GitHub](https://github.com).

## Coding style

The most visible part of a coding style is the naming convention.
People use many different styles of naming convention within the R ecosystem [@Baath2012].
...
RStudio make things easier when you activate all diagnostic options (_Tools_ > _Global options_ > _Code_ > _Diagnostics_).
...
Doing so enforces you to consistenly use the same coding style, making it easy to learn and use it.
...
- Use `seq_len()` or `seq_along()` instead of `1:length(x)`, `1:nrow(x)`, ...
  When `length(x) == 0`, `1:length(x)` yields `c(1, 0)`, whereas `seq_along(x)` would yield an empty vector.
...
- Avoid code with lots of nested loop or if statements.
  If the code it too complex, you'll get a warning that the [cyclomatic complexity](https://en.wikipedia.org/wiki/Cyclomatic_complexity) is too high.
...
When `check_filename()` does not allow a certain file or folder name and you think this is a false positive, then please open an [issue on GitHub](https://github.com/inbo/checklist/issues) and motivate why this is a false positive.
...
- Same rules except that you need to use a dash (`-`) as separator instead of an underscore (`_`).
  We need to exception because underscores cause problems in certain situations.

## Bundeling your code a package

Most users think of an R package a collection of generic functions that they can use to run their analysis.
However, an R package is useful way to bundle and document a stand alone analysis too!
Suppose you want to pass your code to a collaborator or your future self who is working on a different computer.
If you have a as project folder with a bunch of files, people will need to get to know your project structure, find out what scripts to run and which dependencies they need.
Unless you documented everything well they (including your future self!) will a hard time figuring out how thing work.

Having the analysis as a package _and_ running `check_package()` to ensure a minimal quality standard, make things a lot easier for the user.
...
- Functions make it easy to re-use code.
  Need to run the same thing with a different parameter values?
  ...
  Image you made a mistake in the code and copy-pasted that mistake several times before you found it.
  You have to check your entire project to fix the mistake.
...
- Packages require that every object is either defined within the package or imported from another package.
  ...
  This ensures that the functions inside your package only work with the objects defined inside the package.
  They are not masked by objects defined by the user.
  Something that can happen in standalone scripts.
- A package forces you to write (minimal) documentation.
  Every function needs a least a title and an entry for every argument.
- Most likely should still need a short script that combines a few high level functions of your package to run the analysis.
