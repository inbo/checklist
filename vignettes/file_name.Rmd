---
title: "Naming conventions for folders and files"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Naming conventions for folders and files}
...
When `check_filename()` triggers a false positive for a certain file or folder name, then please open an [issue on GitHub](https://github.com/inbo/checklist/issues) and motivate why this should be allowed.
...
```{r, include = FALSE}
extensions <- sprintf("`%s`", checklist:::graphics_ext)
head(extensions, -1) |>
  paste(collapse = ", ") |>
  paste("and", tail(extensions, 1)) -> extensions
```


- Currently applies to files with these extensions: `r extensions`.
- Same rules except that you need to use a dash (`-`) as separator instead of an underscore (`_`).
  We need this exception because underscores cause problems with graphics files in certain situations.
