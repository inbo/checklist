test_that("clean_git with `main` as main branch", {
  ...
  expect_identical(
    ...
    branch_info_repo2$commit[branch_info_repo2$name == "origin/branch"] # nolint: nonportable_path_linter
  )
  ...
  gert::git_branch_set_upstream(upstream = "origin/branch", repo = repo2) # nolint: nonportable_path_linter
  expect_invisible(clean_git(repo = repo2, verbose = FALSE))
  branch_info_repo <- git_branch_list(repo = repo)
  branch_info_repo2 <- git_branch_list(repo = repo2)
  expect_identical(
  ...
  ab <- git_ahead_behind(
    upstream = "origin/branch", ref = "branch", repo = repo2 # nolint: nonportable_path_linter
  )
  ...
  branch_info_repo <- git_branch_list(repo = repo)
  gert::git_merge(ref = "branch", commit = TRUE, squash = FALSE, repo = repo)
  git_push(repo = repo)
