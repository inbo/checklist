...
- Many citation-related functions have been moved to the new
  [`citeme`](https://github.com/inbo/citeme) package (\>= 0.1.1). The
  following functions are no longer part of `checklist`:
  `citation_meta()`, `default_organisation()`,
  `get_available_organisations()`, `get_default_org_list()`,
  `new_org_list()`, `org_item`, `org_list`, `organisation`,
  `read_organisation()`, `store_authors()`, `upload_zenodo()`,
  `use_author()`, `write_citation_cff()`, `write_organisation()`, and
  `write_zenodo_json()`.
- `bookdown_zenodo()` is removed.
- `add_badges()` is renamed to `add_checklist_badges()`.
- `check_codemeta()` is removed because it provided not much extra
  information. This also allows us to remove a dependency.
- `create_draft_pr()` and `get_branches_tags()` are removed to remove
  the dependency on `gh`. Note that we now have a GitHub template for
  pull requests.

### New functions

- [`add_agents()`](https://inbo.github.io/checklist/reference/add_agents.md)
  installs AI agent configuration files (`.github/agents/`) in a package
  repository.
  ...
  now call this function automatically.
- [`add_issue_templates()`](https://inbo.github.io/checklist/reference/add_issue_templates.md)
  adds GitHub issue and pull request templates to a package repository.
  [`create_package()`](https://inbo.github.io/checklist/reference/create_package.md)
  and
  [`setup_package()`](https://inbo.github.io/checklist/reference/setup_package.md)
  now call this function automatically.
- `fix_duplicate_git_config()` removes duplicate entries from the local
  git config, fixing issues that could prevent
  [`clean_git()`](https://inbo.github.io/checklist/reference/clean_git.md)
  from working ([\#191](https://github.com/inbo/checklist/issues/191)).

### Other changes

- [`check_package()`](https://inbo.github.io/checklist/reference/check_package.md)
  gains a `timeout` argument to control the maximum time allowed for the
  CRAN checks.
- [`check_cran()`](https://inbo.github.io/checklist/reference/check_cran.md)
  now displays the full output of all failing tests.
- [`check_filename()`](https://inbo.github.io/checklist/reference/check_filename.md)
  allows `Makevars`, `Cargo.toml`, `Cargo.lock`, and `*.agent.md` file
  names.
- [`check_lintr()`](https://inbo.github.io/checklist/reference/check_lintr.md)
  ignores the `tests/testthat/_problems` folder.
- [`check_spelling()`](https://inbo.github.io/checklist/reference/check_spelling.md)
  improves the handling of Markdown URLs
  ([\#190](https://github.com/inbo/checklist/issues/190)) and world
  clock time-out errors.
- Submodule directories are ignored when running
  [`check_lintr()`](https://inbo.github.io/checklist/reference/check_lintr.md),
  [`check_filename()`](https://inbo.github.io/checklist/reference/check_filename.md),
  [`check_folder()`](https://inbo.github.io/checklist/reference/check_folder.md),
  and
  [`check_spelling()`](https://inbo.github.io/checklist/reference/check_spelling.md)
  ([\#142](https://github.com/inbo/checklist/issues/142)).
- Updated compatibility with `roxygen2` \>= 8.0.0.
- Add the [`CRANhaven`](https://www.cranhaven.org/) repository to reduce
  the impact of recently archived CRAN packages.
- [`clean_git()`](https://inbo.github.io/checklist/reference/clean_git.md)
  handles duplicate entries in `git config`.
...
- `organisation` class and its related function like
  `default_organisation()`, `read_organisation()` and
  `write_organisation()` are deprecated.
- The new `org_list` enforces the use of the INBO ROR
  ([\#153](https://github.com/inbo/checklist/issues/153)).

### New functions

- `get_default_org_list()` returns the default organisation list
  depending on the git remote `origin`.
...
- New `organisation()` class to hold the requirements of the
  organisation. For the time being this is hard-coded to the Research
  Institute for Nature and Forest (INBO).
- Author affiliations must match one of the affiliations set in
  `organisation()`. The membership of an author is determined by their
  e-mail or their affiliation. This is checked when creating or using
  author information and when updating citation information.
...
- `citation_meta()` gains support for
  [`bookdown`](https://pkgs.rstudio.com/bookdown/) reports.
- Add `bookdown_zenodo()` which first extracts the citation metadata
  from the yaml header. Then it cleans the output folder and renders the
  required output formats. Finally it uploads the rendered files to a
  draft deposit on [Zenodo](https://zenodo.org).
...
- Fix problem in `write_zenodo_json()` which produced a `.zenodo.json`
  which failed to parse on <https://zenodo.org>.
- `write_zenodo_json()` and `write_citation_cff()` return the checklist
...
- Package require a `codemeta.json` as written by
  `codemetar::write_codemeta`. Suggestions by `codemetar` to improve the
  package become checklist notes.
- [`set_tag()`](https://inbo.github.io/checklist/reference/set_tag.md)
